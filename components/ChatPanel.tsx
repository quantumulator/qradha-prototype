...
import { Send, X, Loader2, Sparkles, Settings, Key, Check, AlertCircle, Zap, Brain, Shield, FileText } from 'lucide-react';
...
export default function ChatPanel() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const useGroq = apiKeySet && groqAgent.isConfigured();
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {apiKeySet ? 'Groq Connected' : 'Demo Mode'}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {quickActions.map((action) => (
                <button
                  ...
                  className="text-xs px-2.5 py-1.5 rounded-lg bg-navy-400 hover:bg-navy-300 text-gray-400 hover:text-white transition-all hover:scale-105"
                >
                  {action.label}
                </button>
              ))}
